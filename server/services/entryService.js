...
async function updateEntryService(user_id, entry_id, { content }) {
    ...
    try {
        ...
        if (entry.is_deleted === true) {
            throw new Error("Silinmiş entry güncellenemez.");
        }

        if (Number(entry.user_id) !== user_id) {
            throw new Error("Bu entry'i güncelleme yetkiniz yok.");
        }

        if (entry.type === "REPOST") {
            throw new Error("Repost entry güncellenemez.");
        }
        ...
        return await hydrateEntryCardByEntryIdService(entry_id, user_id);
    } catch (error) {
        await client.query("ROLLBACK");
        throw error;
    } finally {
